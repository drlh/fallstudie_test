...
public class Spiel
{
    ...
    public final static int MAX_RUNDEN = 12;
	...
	if (i < spieler.size() - 1 && aktuelleRunde < MAX_RUNDEN) {
	    getAktuelleRunde().setAktuellerSpielerNr(i + 1);
	    Spiel.aktuellerSpielerName = getAktuelleRunde()
		    .getAktuellerSpieler().getName();
	} else if (i >= spieler.size() - 1 && aktuelleRunde < MAX_RUNDEN) {
	    neueRunde();
	} else if (aktuelleRunde >= MAX_RUNDEN) {
	    spielBeenden();
	}
